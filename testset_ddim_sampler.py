...
import sys
import os
sys.path.append('/root/SGDiff') # For ldm
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'stage2')) # For stage2_utils
...
def get_model(sg_ckpt=None):
    config = OmegaConf.load("./config_vg.yaml")
    
    # Override VQGAN path as requested by user
    config.model.params.first_stage_config.params.ckpt_path = "/root/Scene-Graph-Guided-Latent-Editing/pretrained/model.ckpt"
    
    if sg_ckpt is not None:
        print(f"Overriding Scene Graph Checkpoint: {sg_ckpt}")
        config.model.params.cond_stage_config.params.ckpt_path = sg_ckpt
    model = load_model_from_config(config, "/root/Scene-Graph-Guided-Latent-Editing/pretrained/model.ckpt")
    return model
...
def main():
    ...
    parser.add_argument("--sg_ckpt", type=str, default=None, help="Path to Scene Graph checkpoint (overrides config_vg.yaml)")
    args = parser.parse_args()

    model = get_model(sg_ckpt=args.sg_ckpt)
    ...
    with torch.no_grad():
        with model.ema_scope():
            ...
            for batch_data in loader:
                ...
                draw_scene_graph_matplotlib(objs, triples, vocab, scene_graph_path)                
